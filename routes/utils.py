...
def get_routes(request, form) -> dict:
    ...
    if not len(all_ways):
        raise ValueError('Маршрута не найдено')
    if cities:
        ...
        if not right_ways:
            raise ValueError('Маршрута через эти города не найдено')
    else:
        right_ways = all_ways
    trains = []
    all_trains = {}
    ...
    if not trains:
        print(trains)
        print(tmp['total_time'])
        raise ValueError('Время в пути больше задонного')
    return context
